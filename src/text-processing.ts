...
- 文字起こしは「無音から湧いた語」ではなく「発話が音の近い別表記に化けたもの」とみなしてください。意味が通じない内容語（名詞・カタカナ・英数字混じり等）を削除しないでください。削除してよいのは意味を持たないフィラーや明らかな重複のみです。
- 音が一致または酷似し、文脈上自然な既知の固有名詞・技術用語・決まり文句がある場合は、そちらに寄せて復元してください。確信が持てない場合も削除せず原文のまま残し、数字・日付・金額だけは推測で書き換えないでください。
...
- Treat transcription errors as phonetic misrecognitions of something actually spoken, not words appearing from silence. Never delete an unintelligible content word (noun, katakana, alphanumeric mix, etc.). Only semantically empty fillers and obvious repetitions may be deleted.
- When a known proper noun, technical term, or set phrase sounds identical or highly similar and fits the context, restore it toward that form. If uncertain, keep the original instead of deleting; only numbers, dates, and money amounts must never be rewritten by guess.
